...
public class FoodViewer {
	...
	public void manageFood() {
		try {
			...
			System.out.println("---------------------------------------------------------");
			System.out.println("[" + Menu.F_SHOW_ALL + "] 현재 음식 목록");
			System.out.println("[" + Menu.F_INSERT + "] 새 음식 등록");
			System.out.println("[" + Menu.F_EXIT + "] 뒤로 가기");
	...
	private void update(int fid) {
		...
		private void foodManager(List<Food> list) {
			...
			while(true) {
			System.out.println("[" + Menu.F_UPDATE + "] 음식 정보 변경");
			System.out.println("[" + Menu.F_DELETE + "] 음식 정보 삭제");
			System.out.println("[" + Menu.F_EXIT + "] 뒤로 가기");
